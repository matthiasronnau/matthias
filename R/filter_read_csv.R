...
filter_read_csv <- function(path, batch_size, filters){
  ...
  cols.names <- colnames(readr::read_csv(path, skip = batches[1], n_max = 0, col_types = readr::cols(), progress = FALSE)) #Get column names from data
  ...
  for(i in 1:length(batches)){ #Iterate through every batch
    data_temp <- readr::read_csv(path, skip = batches[i], #Read in a batch_size amount of data
                          n_max = batch_size, col_types = readr::cols(), progress = FALSE)
